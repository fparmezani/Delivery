...
cardapio.method = {
  ...
  obterDadosEmpresa: () => {
    app.method.get(
      ...
      (response) => {
        ...
        document.querySelector("#lblNomeEmpresa").innerText =
          response.data[0].nome;

        if (response.data[0].logotipo != null) {
          document.querySelector(
            "#imgLogoEmpresa"
          ).style.backgroundImage = `url('/public/images/empresa/${response.data[0].logotipo}')`;
          document.querySelector("#imgLogoEmpresa").style.backgroundSize =
            "cover";
        } else {
  ...
  carregarCategorias: (list) => {
    if (list.length > 0) {
      ...
      list.forEach((e, i) => {
        ...
        let temp = cardapio.templates.categoria
          .replace(/\${idcategoria}/g, e.idcategoria)
    ...
    if (list.length > 0) {
      list.forEach((e, i) => {
        ...
        let temp = cardapio.templates.produto
          ...
          .replace(
            /\${valor}/g,
            parseFloat(e.valor).toFixed(2).replace(".", ",")
          );
  ...
  validarCategoriaScroll: () => {
    var categorias = document
      .querySelector("#listaItensCardapio")
