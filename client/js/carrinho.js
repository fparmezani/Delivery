...
carrinho.method = {
    ...
    if (CARRINHO_ATUAL.length > 0) {
      ...
      if (TAXA_ATUAL > 0) {
        ...
        document.querySelector(
          "#lblTaxaEntrega"
        ).innerText = `+ R$ ${parseFloat(TAXA_ATUAL)
          .toFixed(2)
          .replace(".", ",")}`;
      } else {
      ...
      document.querySelector("#lblTotalCarrinho").innerText = `R$ ${parseFloat(
        total
      )
        .toFixed(2)
        .replace(".", ",")}`;
      ...
      ).innerText = `R$ ${parseFloat(total).toFixed(2).replace(".", ",")}`;
    }
  },
    ...
    app.method.get(
      ...
      (response) => {
        ...
        if (delivery[0].ativo) {
          ...
          if (
            delivery[0].tempominimo != null &&
            delivery[0].tempominimo > 0 &&
            delivery[0].tempomaximo != null &&
            delivery[0].tempomaximo > 0
          ) {
        ...
        if (retirada[0] != undefined && retirada[0].ativo) {
          let tempo = "";

          if (
            retirada[0].tempominimo != null &&
            retirada[0].tempominimo > 0 &&
            retirada[0].tempomaximo != null &&
            retirada[0].tempomaximo > 0
          ) {
  ...
  selecionarFormaPagamento: (forma) => {
    let selecionada = FORMAS_PAGAMENTO.filter(
      (e) => e.idformapagamento == forma
    );
    ...
    if (selecionada.length > 0) {
      ...
      // se for dinheiro
        ...
        if (troco != null) {
          document.querySelector(
            "#lblDescFormaPagamentoSelecionada"
          ).innerText = `Troco para: ${troco} reais`;
