...
pedido.method = {
      ...
      app.method.get(
        ...
        (response) => {
          ...
          let temp = pedido.template.dadospedido
            ...
            .replace(
              /\${valor}/g,
              `R$ ${parseFloat(response.data.total)
                .toFixed(2)
                .replace(".", ",")}`
            );
          ...
          if (response.data.idformapagamento == 1) pedido.method.carregarPix();
        },
        (error) => {
  ...
  copieeColePix: () => {
    const el = document.querySelector("#pixCode");
    el.select();
    document.execCommand("copy");
    app.method.mensagem(
      "Código copiado, agora faça o pagamento e envie por Whatsapp",
      "green",
      3500
    );
  },
  ...
  carregarPix() {
    document.querySelector("#containerPix").classList.remove("hidden");
  },
  ...
  carregarModalDetalhes: (data) => {
    ...
    if (ORDER.taxaentrega > 0) {
      ...
      let temptaxa = pedido.template.taxaentrega.replace(
        /\${total}/g,
        `+ R$ ${parseFloat(ORDER.taxaentrega).toFixed(2).replace(".", ",")}`
      );
