  ...
  carregarModalDetalhes: (data, idpedido, cart) => {
    ...
    if (data.idtipoentrega == 1) {
      ...
      document.querySelector("#lblEndereco").innerText = `${data.endereco}, ${
        ...
        data.complemento.length > 0 ? ` (${data.complemento})` : ""
      }`;
      document.querySelector(
        "#lblCep"
      ).innerText = `${data.cidade}-${data.estado} / CEP:${data.cep}`;
    } else {
      // retirada
      ...
      if (data.idpedidostatus == 1) {
        document.querySelector(
          "#container-action-footer"
                      ...
                      <button type="button" class="btn btn-yellow btn-sm" onclick="pedido.method.imprimirPedido()">Imprimir Pedido</button>`;
      }
