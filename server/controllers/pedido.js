...
const controllers = () => {
    ...
    try {
      ...
      if (pedido.cart.length > 0) {
        ...
        if (pedido.taxaentrega > 0) {
          total += parseFloat(pedido.taxaentrega);
        }
      }
  ...
  const obterPedidoPorId = async (req) => {
    try {
      ...
      if (painel) {
        ...
        var ComandoSQLItens = await readCommandSql.retornaStringSql(
          "obterItensPedido",
          "pedido"
        );
  ...
  const obterPedidoPorStatus = async (req) => {
    try {
      ...
      var ComandoSQLTotais = await readCommandSql.retornaStringSql(
        "obterTotaisPedidos",
        "pedido"
      );
