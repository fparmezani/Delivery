...
const controllers = () => {
  ...
  const duplicarProduto = async (req) => {
    try {
      ...
      var ComandoSQLProduto = await readCommandSql.retornaStringSql(
        "obterPorId",
        "produto"
      );
      var dados_produto = await db.Query(ComandoSQLProduto, {
        idproduto: idproduto,
      });
  ...
  const removerProduto = async (req) => {
    try {
      ...
      const produto = await db.Query(ComandoSQLSelect, {
        idproduto: idproduto,
      });
  ...
  const ativarProduto = async (req) => {
    try {
      ...
      return {
        status: "error",
        message: "Falha ao ativar produto. Tente novamente.",
        ex: ex,
      };
    }
  };
